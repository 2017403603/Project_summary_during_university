        ...
        for i in range(0, self.vardim):
            ...
            if random.random()<self.params2[0]:
                if random.random()<self.params2[1]:
                    ind.chrom[i]=self.uu*(self.population[bestIdx].chrom[i]+random.random()*(self.population[random.randint(0,self.sizepop-1)].chrom[i]-self.population[random.randint(0,self.sizepop-1)].chrom[i]+self.population[random.randint(0,self.sizepop-1)].chrom[i]-self.population[random.randint(0,self.sizepop-1)].chrom[i]))
                ...
                for k in range(0,self.sizepop):
                    for kk in range(0,self.vardim):
                        x_max_cov = max(x_max_cov, self.population[k].chrom[kk])
                        x_min_cov = min(x_min_cov, self.population[k].chrom[kk])
