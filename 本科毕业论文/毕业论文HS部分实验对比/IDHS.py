...
class HarmonySearch:
        ...
        for i in range(0,self.vardim):
            ...
            if r1 <=self.params2[0]:
                if r2 <self.params2[1]:
                    ...
                    bestIdx = np.argmin(self.fitness)
                    ind.chrom[i]=self.population[bestIdx].chrom[i]+0.6*(self.population[random.randint(0,self.sizepop-1)].chrom[i]-self.population[random.randint(0,self.sizepop-1)].chrom[i])
    ...
    def solve(self):
        ...
        print("IDHS_best:", self.best.fitness)
