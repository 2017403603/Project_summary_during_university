...
class HarmonySearch:
        ...
        for i in range(0,self.vardim):
            ...
            if r1 <self.params2[0]:
                if r2 <self.params2[1]:
                    ...
                    '''
                    bestIdx = np.argmin(self.fitness)
                    ind.chrom[i]=self.population[bestIdx].chrom[i]+random.random()*(self.population[random.randint(0,self.sizepop-1)].chrom[i]-self.population[random.randint(0,self.sizepop-1)].chrom[i])
                    if ind.chrom[i] < self.bound[0, i]:
                        ind.chrom[i] = self.bound[0, i]
                    if ind.chrom[i] > self.bound[1, i]:
                        ind.chrom[i] = self.bound[1, i]
            else:
                ind.chrom[i] = self.population[random.randint(0,self.sizepop-1)].chrom[i]+0.6*(self.population[random.randint(0,self.sizepop-1)].chrom[i]-self.population[random.randint(0,self.sizepop-1)].chrom[i])
                if ind.chrom[i] < self.bound[0, i]:
                    ind.chrom[i] = self.bound[0, i]
                if ind.chrom[i] > self.bound[1, i]:
                    ind.chrom[i] = self.bound[1, i]
    ...
    def solve(self):
        ...
        print("sci_HSbest:", self.best.fitness)
