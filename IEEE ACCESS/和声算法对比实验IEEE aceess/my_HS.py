...
class HarmonySearch:
        ...
        for i in range(0,self.vardim):
            ...
            if r1 <self.params2[0]:
                if r2 <self.params2[1]:
                    ...
                    #ind.chrom[i] += self.best.chrom[random.randint(0, self.sizepop - 1)]
                else:
                    ...
                    #ind.chrom[i]=self.best.chrom[i]+random.random()*(self.population[random.randint(0,self.sizepop-1)].chrom[i]-self.population[random.randint(0,self.sizepop-1)].chrom[i])
            else:
                ind.chrom[i] = self.population[random.randint(0,self.sizepop-1)].chrom[i]+random.random()*(self.population[random.randint(0,self.sizepop-1)].chrom[i]-self.population[random.randint(0,self.sizepop-1)].chrom[i])
                if ind.chrom[i] < self.bound[0, i]:
                    ind.chrom[i] = self.bound[0, i]
                if ind.chrom[i] > self.bound[1, i]:
                    ind.chrom[i] = self.bound[1, i]
    ...
    def solve(self):
        ...
        print("my_HSbest:", self.best.fitness)
